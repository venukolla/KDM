...
import com.ibm.scientists.Engine;
...
public class SearchController 
{
	...
	public static List<Scientist> scientist = Config.cc.getAllScientistsNotInGroup(Constants.CURRENT_Scientists_GROUP);
	
	/** Returns search results as a JSON string. **/
	@GET
	public Response handleSearch(@Context UriInfo ui)
	{
		System.out.println("Num people: " + scientist.size());
		...
		Engine engine = new Engine(scientist);
		System.out.println("Engine created.");
		
		List<Scientist> results = engine.query(fullName);
	...
	public static Scientist getPersonFromList(String fullname)
	{
		for (Scientist s: SearchController.scientist)
		{
			if (s != null && s.name != null && s.name.equals(fullname))
			{
				return s;
			}
		}
